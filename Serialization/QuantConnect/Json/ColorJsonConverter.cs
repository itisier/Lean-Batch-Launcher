...
namespace Panoptes.Model.Serialization
{
    ...
    public sealed class ColorJsonConverter : JsonConverter<Color>
    {
        ...
        private static int HexToInt(ReadOnlySpan<char> hexValue)
        {
            ...
            {
                var message = $"Unable to convert '{hexValue.ToString()}' to an Integer. Requires string length of 2.";
                throw new FormatException(message);
            }
            ...
            {
                throw new FormatException($"Invalid hex number: {hexValue.ToString()}");
            }
